...
These files are the worked examples behind the scoring tests, and they are documentation as much
as test data: a reader should be able to see what a good score and a bad score look like without
...
- Comments explain *why* — the constraint, tradeoff, or history — rather than restating the code.

Note the comment on the environment-variable precedence in `resolve_config`. It earns credit
rather than merely being tolerated, because explaining a subtle ordering is exactly the work that
...
The point of the file is that it is hard to read *and* hard to test — the two scores fall
together, which is what you would expect of genuinely bad code and what makes the metric
trustworthy.
