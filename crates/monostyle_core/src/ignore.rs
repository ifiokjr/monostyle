...
impl IgnoreConfig {
	...
	pub fn allows(&self, path: &Path) -> bool {
		...
		// Three routes into the same decision: the file's name, which is cheap; its header, which catches
		// a generated file with an ordinary name; and its first line, which catches a checked-in bundle.
		if self.skip_generated()
			&& (is_generated(path) || declares_generated(path) || declares_bundle(path))
		{
			return false;
		}
...
/// Banner phrases that identify a checked-in bundle.
///
/// A bundle is somebody else's code — every line of it — so scoring it describes a dependency rather than
/// the project. These are matched against the first line only, and each is a phrase a bundler writes
/// rather than a bare product name: a hand-written file can legitimately mention `webpack` in a comment,
/// but `esbuild bundle(` is a banner nothing writes by accident.
///
/// The case that prompted this: a repository committed `esm.sh - esbuild bundle(@wallet-standard/app)`
/// output, and the bundle was reported as twelve statements run together and a magic number — in a file
/// nobody in the repository wrote.
const BUNDLE_BANNERS: &[&str] = &[
	"esbuild bundle",
	"webpack jsonp",
	"webpackbootstrap",
	"rollup v",
...
/// Returns true when a file's first line is a bundler banner.
///
/// Only the first line is checked, because that is where a bundler writes its signature. A longer search
/// would start matching ordinary prose that happens to name a build tool.
#[must_use]
pub fn declares_bundle(path: &Path) -> bool {
	...
	BUNDLE_BANNERS
		.iter()
		.any(|banner| lowered.contains(&banner.to_ascii_lowercase()))
}
