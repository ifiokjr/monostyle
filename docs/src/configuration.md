...
Run `monostyle config` to print every available key with its current value. The output is valid TOML and can
be pasted into a `monostyle.toml` file as a starting point.

## Unknown keys

An unrecognized key is an error rather than a warning. A typo in a threshold name would otherwise be silently
accepted, and the project would score against settings nobody chose.
